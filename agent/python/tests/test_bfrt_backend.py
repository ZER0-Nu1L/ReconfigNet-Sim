...
from ocs_agent.backends import bfrt
from ocs_agent.backends.base import BackendTransitionError
from ocs_agent.core import OcsAgent
from ocs_agent.errors import FailedPreconditionError
from ocs_agent.model import ConnectionSet, PortInventory
...
class FakeTable(object):
    ...
    def _pair(key):
        ...
        return (
            values[bfrt.DEFAULT_INGRESS_FIELD],
            values[bfrt.DEFAULT_EGRESS_FIELD])
    ...
    def entry_get(self, target, keys, flags):
        ...
        for ingress, egress in sorted(self.entries):
            yield None, FakeKey([
                FakeTuple(bfrt.DEFAULT_INGRESS_FIELD, ingress),
                FakeTuple(bfrt.DEFAULT_EGRESS_FIELD, egress),
            ])
...
class BFRTBackendTests(unittest.TestCase):
    def setUp(self):
        self.original_loader = bfrt._load_bfrt_client

    def tearDown(self):
        bfrt._load_bfrt_client = self.original_loader

    def backend(self, entries=None, consistency='CACHED_SYNC'):
        table = FakeTable(entries)
        gc = FakeGC(table)
        bfrt._load_bfrt_client = lambda unused=None: gc
        backend = bfrt.BFRTBackend({
            'type': 'bfrt',
